#!/usr/bin/env python3
"""
全サイトの予約投稿を 0:00〜22:00・2時間刻み（正時）の枠に沿って再調整するスクリプト。
api_poster.py の予約枠（1日12枠）と整合させる。
"""
import json
import base64
...
MAX_PER_DAY = 12  # 1日最大12枠（0:00〜22:00・2時間刻み・正時、api_poster と同じ）
...
def reschedule_site(site_name, api_url, user, app_pass, dry_run=True):
    ...
    # 本日から全件を再配置。枠は 0:00〜22:00・2時間刻み（正時）、1 日最大 12 件。
    new_assignments = []
