...
def _ceil_to_next_hour_slot(dt):
    """dt より後の最初の正時 :00 境界（予約枠のフォールバック用）。"""
    dt = dt.replace(second=0, microsecond=0)
...
_schedule_minute = 0  # --minute と併用: 分（0〜59）。--hour 指定時の slot に反映
...
def main():
    ...
    if not _post_as_draft and scheduled_date is None:
        ...
        now = datetime.now()
        # 予約枠：空き検索時は1日24件（毎正時 :00）。--hour/--minute 指定時は分まで含めて占有判定
        ...
        if not scheduled_date:
            for day_offset in range(day_range):
                ...
                for h in fallback_hours:
                    slot_candidate = current_day.replace(hour=h, minute=0, second=0, microsecond=0)
        ...
        if not scheduled_date:
            scheduled_date = _ceil_to_next_hour_slot(now + timedelta(minutes=1)).strftime("%Y-%m-%dT%H:%M:%S")
